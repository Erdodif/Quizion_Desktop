...
namespace MultiLanguageMarkup
{
  ...
  internal class LangExtension : MarkupExtension
  {
    ...
    private static string RootNamespace = "Projekt";     //MLHIDE
    ...
    // The design culture may be set via the inherited attached property Design.Culture.
    // _TargetDepOb is the object on which we read the Design.Culture property.
    private CultureInfo             _DesignCulture ;
    private DependencyObject        _TargetDepOb ;
    ...
    public override object ProvideValue (IServiceProvider serviceProvider)
    {
	  ...
	  var targetProperty = provider.TargetProperty as DependencyProperty ;

      // In design mode only, get the (inherited) attached property m:Design.Culture and
      // add a property changed handler to catch changes to this property.
      if ( provider.TargetObject is DependencyObject targetDepOb )
      {
        if ( DesignerProperties.GetIsInDesignMode ( targetDepOb ) )
        {
          _TargetDepOb   = targetDepOb ;
          _DesignCulture = (CultureInfo)_TargetDepOb.GetValue ( Design.CultureProperty ) ;
          var dpd = DependencyPropertyDescriptor.FromProperty ( Design.CultureProperty, _TargetDepOb.GetType() );
          dpd.AddValueChanged ( _TargetDepOb, DesignCultureChanged ) ;
        }
      }

      if ( provider.TargetObject is FrameworkElement targetElement )
      {
		targetElement.Unloaded += Element_Unloaded;
        targetElement.Loaded   += Element_Loaded;
        _Targets.Add ( new target { elem = targetElement, depobj = targetElement, depprop = targetProperty, loaded = false } ) ;
      }
      else if ( provider.TargetObject is FrameworkContentElement contentElement )
      {
        ...
        if ( contentElement.Parent is FrameworkElement parentElement )
        {
          parentElement.Unloaded += Element_Unloaded;
          parentElement.Loaded   += Element_Loaded;
          _Targets.Add ( new target { elem = parentElement, depobj = contentElement, depprop = targetProperty, loaded = false } );
        }
      }
      ...
      return ResMgr.GetString ( _ResourceKey, _DesignCulture ) ;
    }
    ...
    protected virtual void ml_UpdateControls()
    {
      ...
      foreach ( var t in _Targets )
      {
        if ( t.loaded )
        {
          ...
          {
            var resString = ResMgr.GetString ( _ResourceKey, _DesignCulture ) ;
    ...
    private void DesignCultureChanged ( object sender, EventArgs e )
    {
      _DesignCulture = (CultureInfo)_TargetDepOb.GetValue ( Design.CultureProperty ) ;
      ml_UpdateControls() ;
    }
  ...
  //
  // Attached property Design.Culture
  // --------------------------------
  // This class defines the attached property Culture.
  // If you define the attribute (for example)
  //   m:Design.Culture="de"
  // at the top level of your XAML file, it will set the culture for strings
  // returned by the markup extension m:Lang in design mode.
  //
  // This attached property is defined with the metadata "Inherits" which means
  // it is passed down to all lower level objects in the XAML tree.
  //
